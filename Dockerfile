# 多阶段构建 Dockerfile for CineMaker
...
FROM ${DOCKER_REGISTRY:-}node:22-alpine AS frontend-builder
...
FROM ${DOCKER_REGISTRY:-}golang:1.25-alpine AS backend-builder
...
# 复制 Go 模块文件（单独层，只要 go.mod/go.sum 不变就缓存）
COPY go.mod go.sum ./
RUN go mod download

# 复制后端源码（逐目录复制，前端变更不会破坏此层缓存）
COPY *.go ./
COPY api/ ./api/
...
RUN CGO_ENABLED=0 go build -ldflags="-w -s" -o cinemaker .
...
# 从构建阶段复制可执行文件
COPY --from=backend-builder /app/cinemaker .
