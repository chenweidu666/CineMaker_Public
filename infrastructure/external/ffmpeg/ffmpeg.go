...
// ExtractLastFrame 从视频中截取最后一帧为 JPEG 图片
// videoPath: 本地视频文件路径（绝对路径）
// outputPath: 输出图片路径（绝对路径，.jpg）
func (f *FFmpeg) ExtractLastFrame(videoPath, outputPath string) error {
	f.log.Infow("Extracting last frame from video", "video", videoPath, "output", outputPath)
	...
	// 直接从视频末尾截取最后一帧，使用 -sseof 参数（从末尾向前偏移）
	// -sseof -0.1 表示从视频末尾向前 0.1 秒的位置
	cmd := exec.Command("ffmpeg",
		"-sseof", "-0.1",
		"-i", videoPath,
		"-frames:v", "1",
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		f.log.Errorw("FFmpeg last frame extraction failed", "error", err, "output", string(output))
		return fmt.Errorf("ffmpeg extract last frame failed: %w, output: %s", err, string(output))
	}

	// 验证输出文件存在且非空
	info, err := os.Stat(outputPath)
	if err != nil || info.Size() == 0 {
		return fmt.Errorf("extracted frame is empty or missing")
	}

	f.log.Infow("Last frame extracted successfully", "output", outputPath, "size", info.Size())
	return nil
}
