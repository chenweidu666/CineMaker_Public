...
func SafeParseAIJSON(aiResponse string, v interface{}) error {
	...
	// 4.5 如果还是失败，尝试把 {}, {} 包装成数组 [{}, {}]
	// AI 有时返回被截断的数组，regex 提取后变成 {...}, {...} 格式
	if strings.Contains(err.Error(), "after top-level value") {
		wrappedJSON := "[" + jsonMatch + "]"
		wrappedFixed := attemptJSONRepair(wrappedJSON)
		if err2 := json.Unmarshal([]byte(wrappedFixed), v); err2 == nil {
			return nil // 包装为数组后解析成功
		}
		// 如果被截断，尝试去掉最后一个不完整的对象
		if lastComplete := strings.LastIndex(wrappedFixed, "},"); lastComplete > 0 {
			trimmed := wrappedFixed[:lastComplete+1] + "]"
			if err3 := json.Unmarshal([]byte(trimmed), v); err3 == nil {
				return nil // 去掉截断部分后解析成功
			}
		}
	}
