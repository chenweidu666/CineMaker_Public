<template>
  <div class="character-images-container">
        ...
        <el-row :gutter="20">
          <el-col :span="6" v-for="character in characters" :key="character.id">
            ...
            >
              ...
              <el-button
                type="primary"
                @click="showGenerateDialog(character)"
                :disabled="
                  batchGenerating ||
    ...
    <el-dialog
      ...
      :title="`生成${currentCharacter?.name}的形象`"
      width="600px"
    >
      <el-form label-position="top">
        <el-form-item label="AI提取提示词">
          <el-input
            v-model="currentPrompt"
            type="textarea"
            :rows="6"
            placeholder="请输入AI提取提示词，留空则使用默认提示词"
          />
        </el-form-item>
        <el-form-item>
          <el-text type="info" size="small">
            提示词将用于AI生成人物图片，可以手动修改以获得更好的效果
          </el-text>
        </el-form-item>
      </el-form>
      <template #footer>
        ...
        <el-button
          ...
          :loading="generatingIds.includes(currentCharacter?.id)"
        >
          确认生成
        </el-button>
      </template>
...
import { Edit, Picture, Loading } from "@element-plus/icons-vue";
...
const generateImage = async (character: Character, customPrompt?: string) => {
...
const showGenerateDialog = (character: Character) => {
...
const confirmGenerate = async () => {
  ...
  await generateImage(currentCharacter.value, currentPrompt.value);
...
.character-preview .image-loading {
  width: 100%;
  height: 180px;
  display: flex;
  align-items: center;
