        ...
        >
          <template #actions>
            ...
            <ActionButton
              :icon="Download"
              tooltip="导出 ZIP"
              @click="handleExportDrama(drama.id)"
            />
            <el-popconfirm
        ...
        >
          ...
          <el-form-item :label="$t('drama.style')" required>
            ...
            >
              ...
              <el-option :label="$t('drama.styles.comic')" value="comic" />
            </el-select>
          </el-form-item>
        </el-form>
        <template #footer>
...
import { dramaExportImportAPI } from "@/api/videoAnalysis";
...
const handleImportFile = async (e: Event) => {
  ...
  input.value = "";
  try {
    const res = await dramaExportImportAPI.importDrama(file);
    ElMessage.success("导入成功");
    loadDramas();
    router.push(`/dramas/${res.drama_id}`);
  } catch (err: any) {
    ElMessage.error(err?.response?.data?.error?.message || err?.message || "导入失败");
  }
};

const handleExportDrama = (id: string) => {
  dramaExportImportAPI.exportDrama(Number(id));
};
