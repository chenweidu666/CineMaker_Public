  ...
  <div class="video-timeline-editor">
    ...
    <div class="editor-toolbar">
      ...
      <div class="toolbar-right">
        <!-- V3: 一键生成提示词和一键生成图片暂时隐藏，分镜结果直接作为视频提示词 -->
        <!-- <el-button
          type="info"
          :icon="EditPen"
          @click="emit('batch-generate-prompts')"
          :loading="batchPromptGenerating"
          plain
        >
          一键AI提取提示词
        </el-button>
        <el-button
          type="success"
          :icon="Picture"
          @click="emit('batch-generate-images')"
...
import { getVideoUrl, fixImageUrl } from '@/utils/image'
...
const emit = defineEmits<{
  ...
  (e: 'batch-generate-prompts'): void
  (e: 'batch-generate-images'): void
...
const availableStoryboards = computed(() => {
  const assets = (props.assets || [])
    ...
    .map((a) => ({
      ...
      video_url: getVideoUrl(a),
      thumbnail_url: a.thumbnail_url ? fixImageUrl(a.thumbnail_url) : '',
...
const preloadPlayer = ref<HTMLVideoElement | null>(null)
...
const handlePreviewTimeUpdate = () => {
  ...
  // 预加载下一个视频（在当前视频播放到还剩1秒时）
  const timeRemaining = currentClip.end_time - videoTime
  if (timeRemaining <= 1.0 && timeRemaining > 0.9) {
    ...
    if (nextClip && preloadPlayer.value && preloadPlayer.value.src !== nextClip.video_url) {
      preloadPlayer.value.src = nextClip.video_url
    }
  }
...
const switchToClip = async (clip: TimelineClip) => {
  ...
  // 检查预加载的视频是否已经准备好
  const isPreloaded = preloadPlayer.value && preloadPlayer.value.src === clip.video_url && preloadPlayer.value.readyState >= 3

  // 切换视频源
  currentTime.value = clip.position

  if (isPreloaded) {
    // 如果预加载的视频已经准备好，直接切换，无需等待
...
const getVideoDuration = (videoUrl: string, timeoutMs = 5000): Promise<number> => {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      video.remove()
      reject(new Error('Load video metadata timeout'))
    }, timeoutMs)
...
const addAllScenesInOrder = async () => {
  console.log('[addAll] assets prop count:', props.assets?.length, 'availableStoryboards:', availableStoryboards.value.length)
  ...
  try {
    ...
    console.log('[addAll] sorted scenes:', sortedScenes.map(s => ({ id: s.id, num: s.storyboard_number, url: s.video_url?.substring(0, 60) })))
    ...
    const durations = await Promise.all(
      sortedScenes.map(async (scene) => {
        if (scene.video_url) {
          try {
            return await getVideoDuration(scene.video_url)
          } catch {
    ...
    console.log('[addAll] durations:', durations)

    // 清空当前选中，让所有场景都添加到末尾
    selectedClipId.value = null

    // 批量构建 clips，一次性添加
    let position = timelineClips.value.length > 0
      ? timelineClips.value[timelineClips.value.length - 1].position + timelineClips.value[timelineClips.value.length - 1].duration
      : 0
    ...
    for (let i = 0; i < sortedScenes.length; i++) {
      ...
      const newClip: TimelineClip = {
        id: `clip_${Date.now()}_${i}_${scene.id}`,
        storyboard_id: scene.storyboard_id,
        storyboard_number: scene.storyboard_number,
        video_url: scene.video_url,
        thumbnail_url: scene.thumbnail_url,
        ...
        duration: dur,
        position,
        order: timelineClips.value.length + i,
    ...
    console.log('[addAll] done, timeline clips:', timelineClips.value.length)
...
const downloadAllVideos = async () => {
  if (availableStoryboards.value.length === 0) return
  downloadingAll.value = true
  try {
    for (const scene of availableStoryboards.value) {
      ...
      const shotNum = scene.storyboard_num || scene.storyboard_id || scene.asset_id
      a.download = `shot_${shotNum}.mp4`
    ...
    ElMessage.success(`已下载 ${availableStoryboards.value.length} 个视频`)
  } catch (error: any) {
