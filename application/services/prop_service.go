...
func (s *PropService) UpdateProp(id uint, updates map[string]interface{}) error {
	if referenceImages, ok := updates["reference_images"]; ok {
		if arr, ok := referenceImages.([]interface{}); ok {
			jsonData, err := json.Marshal(arr)
			if err != nil {
				return fmt.Errorf("failed to marshal reference_images: %w", err)
			}
			updates["reference_images"] = datatypes.JSON(jsonData)
		} else if str, ok := referenceImages.(string); ok {
			jsonData, err := json.Marshal([]string{str})
			if err != nil {
				return fmt.Errorf("failed to marshal reference_images: %w", err)
			}
			updates["reference_images"] = datatypes.JSON(jsonData)
...
func (s *PropService) GeneratePropImage(propID uint, promptOverride string, referenceImages []string) (string, error) {
	// 1. 获取道具信息
	...
	// 使用传入的 prompt 覆盖，如果没有则使用数据库中的值
	effectivePrompt := promptOverride
	if effectivePrompt == "" && prop.Prompt != nil {
		effectivePrompt = *prop.Prompt
	}
	if effectivePrompt == "" {
	...
	go s.processPropImageGeneration(task.ID, prop, effectivePrompt, referenceImages)
	return task.ID, nil
}

func (s *PropService) processPropImageGeneration(taskID string, prop models.Prop, prompt string, referenceImages []string) {
	s.taskService.UpdateTaskStatus(taskID, "processing", 0, "正在生成图片...")

	// 查询drama获取风格
	var drama models.Drama
	dramaStyle := "realistic"
	if err := s.db.First(&drama, prop.DramaID).Error; err == nil && drama.Style != "" {
	...
	styleName := styleNameMap[dramaStyle]
	if styleName == "" {
		styleName = dramaStyle + "风格"
	}

	if !strings.Contains(prompt, "纯白") && !strings.Contains(prompt, "纯色") && !strings.Contains(prompt, "white background") {
		prompt = fmt.Sprintf("4:3横向构图，%s，道具三视图设定图。纯白色背景，无场景无装饰无光影。"+
			"一张图内从左到右并排展示同一道具的三个视角：正面全貌、3/4侧面、背面或内部细节特写。"+
			"各视角外观材质配色完全一致。道具描述：%s", styleName, prompt)
	}
