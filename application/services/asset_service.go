...
func (s *AssetService) ImportFromVideoGen(videoGenID uint) (*models.Asset, error) {
	...
	if err := s.db.Where("video_gen_id = ?", videoGenID).First(&existing).Error; err == nil {
		s.log.Infow("Asset already exists for video generation, returning existing", "video_gen_id", videoGenID, "asset_id", existing.ID)
		return &existing, nil
	}
